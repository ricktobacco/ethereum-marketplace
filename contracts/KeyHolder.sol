...
contract KeyHolder is ERC725 
{
    ...
    constructor() public {
        ...
        bytes32 _key = keccak256(tx.origin);
        keys[_key].key = _key;
        keys[_key].purpose = 1;
        keys[_key].keyType = 1;
    ...
    function getKeyPurpose(bytes32 _key)
        public view returns(uint256 purpose) { 
        return (keys[_key].purpose);
    }

    function getKeysByPurpose(uint256 _purpose)
        public view returns(bytes32[] _keys) {
    ...
    function approve(uint256 _id, bool _approve)
        public returns (bool success) {
        require(keyHasPurpose(keccak256(msg.sender), 1), "Sender does not have action key");

        emit Approved(_id, _approve);

        if (_approve == true) {
            executions[_id].approved = true;
            //forward Ether with the transaction to execute. 
